import csv
import os
from tkinter import E, IntVar, N, S, Tk, W, ttk
...
class taylorswiftGUI:
    def __init__(self):
        ...
        self.texts = np.loadtxt(os.path.join(
            taypackagedir, 'answertext.txt'), delimiter='#', dtype='str')
    ...
    def ask_questions(self):
        ...
        for (i, question) in enumerate(answers):
            print(question)
            self.relhap[i] = int(
                input("enter a number from 1-7 representing your mood: "))-4
    ...
    def calculate_song(self):
        ...
        selferr = np.array([self.selffeel[i]-rel[0].get()
                           for i in range(0, self.numsongs)])
        stageserr = np.array([self.stages[i]-rel[1].get()
                             for i in range(0, self.numsongs)])
        seriouserr = np.array([self.seriousness[i]-rel[2].get()
                              for i in range(0, self.numsongs)])
        futureerr = np.array([self.future[i]-rel[3].get()
                             for i in range(0, self.numsongs)])
        maleerr = np.array([self.malefeel[i]-hap[0].get()
                           for i in range(0, self.numsongs)])
        togethererr = np.array([self.together[i]-hap[1].get()
                               for i in range(0, self.numsongs)])
        ...
        for _ in range(0, self.numsongs):
            neterr = selferr**2.+stageserr**2.+seriouserr**2. + \
                futureerr**2.+maleerr**2.+togethererr**2.
